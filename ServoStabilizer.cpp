...
ServoStabilizer::ServoStabilizer(int pitchPin, int rollPin, int yawPin, int throttlePin) {
...
void ServoStabilizer::update() {
  ...
  yawIntegral += yawError * (millis() - lastTime) / 1000.0;
  ...
  yawDerivative = (yawError - lastYawError) / (millis() - lastTime) * 1000.0;
...
void ServoStabilizer::setAngles(float pitchAngle, float rollAngle, float yawAngle) {
...
float ServoStabilizer::outputsYaw() {
  z = yawOutput;
  return z;
}